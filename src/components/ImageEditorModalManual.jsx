...
const ImageEditorModal = ({ isOpen, imageFile, onSave, onCancel }) => {
  ...
  const addLine = () => {
    const line = new fabric.Line([100, 100, 250, 100], {
      stroke: color,
      strokeWidth: 4,
  ...
  const addArrow = () => {
    const arrowPath = "M 0 10 L 100 10 M 80 0 L 100 10 L 80 20";
  ...
  }, [isOpen, canUndo, canRedo]); // eslint-disable-line
      ...
      <div className="glass-panel" style={{
        ...
        <div style={{
          ...
          <div ref={dropdownRef} style={{ position: 'relative' }}>
            <ToolBtn 
              active={showShapesDropdown}
              onClick={() => setShowShapesDropdown(!showShapesDropdown)} 
              icon={<Square size={18} />} 
              label={t('image_editor.shapes')} 
            />
            {showShapesDropdown && (
              <div style={{
                position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', zIndex: 10000,
                marginTop: '0.5rem', background: 'rgba(15, 15, 15, 0.95)',
                ...
                <button 
                  onClick={() => { addRect(); setShowShapesDropdown(false); }} 
                  style={{
                    display: 'flex', alignItems: 'center', gap: '0.6rem',
                    padding: '0.6rem 0.8rem', background: 'transparent',
                    border: 'none', color: 'var(--text-main, #fff)', cursor: 'pointer',
                    width: '100%', borderRadius: '6px', fontSize: '0.85rem',
                    transition: 'background 0.2s', textAlign: 'right'
                  }}
                  className="hover:bg-white/5"
                >
                  <Square size={16} className="text-accent" />
                  <span>{t('image_editor.square')}</span>
                </button>
                <button 
                  onClick={() => { addCircle(); setShowShapesDropdown(false); }} 
                  style={{
                    display: 'flex', alignItems: 'center', gap: '0.6rem',
                    padding: '0.6rem 0.8rem', background: 'transparent',
                    border: 'none', color: 'var(--text-main, #fff)', cursor: 'pointer',
                  ...
                  className="hover:bg-white/5"
                >
                  <CircleIcon size={16} className="text-accent" />
                  <span>{t('image_editor.circle')}</span>
                </button>
                <button 
                  onClick={() => { addArrow(); setShowShapesDropdown(false); }} 
                  style={{
                    display: 'flex', alignItems: 'center', gap: '0.6rem',
                    padding: '0.6rem 0.8rem', background: 'transparent',
                    border: 'none', color: 'var(--text-main, #fff)', cursor: 'pointer',
                ...
                >
                  ...
                  <span>{t('image_editor.arrow')}</span>
                </button>
                <button 
                  onClick={() => { addLine(); setShowShapesDropdown(false); }} 
                  style={{
                    display: 'flex', alignItems: 'center', gap: '0.6rem',
                    padding: '0.6rem 0.8rem', background: 'transparent',
                    border: 'none', color: 'var(--text-main, #fff)', cursor: 'pointer',
        ...
        {showFilters && (
          <div style={{
            ...
            display: 'flex', gap: '2rem', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap'
          }}>
            ...
            <button 
              ...
              style={{
                ...
                color: 'var(--text-main, #fff)', fontSize: '0.8rem',
                transition: 'background 0.2s'
              }}
              className="hover:bg-white/10"
            >
              <RotateCcw size={14} />
              <span>{t('print.setup.reset') || 'إعادة تعيين'}</span>
            </button>
          </div>
        )}
