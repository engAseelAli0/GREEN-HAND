...
const getColorsCount = (orderData) => {
  if (!orderData) return 0;
  if (orderData.numberOfColors && parseInt(orderData.numberOfColors, 10) > 0) {
    return parseInt(orderData.numberOfColors, 10);
  }
  if (orderData.colorDistribution && typeof orderData.colorDistribution === 'object') {
    const keys = Object.keys(orderData.colorDistribution).filter(k => k && String(k).trim() !== '');
    if (keys.length > 0) return keys.length;
  }
  if (Array.isArray(orderData.colors) && orderData.colors.length > 0) {
    return orderData.colors.length;
  }
  if (typeof orderData.colors === 'string' && orderData.colors.trim()) {
    return orderData.colors.split(/[,،\n]+/).filter(Boolean).length;
  }
  if (Array.isArray(orderData.selectedColors) && orderData.selectedColors.length > 0) {
    return orderData.selectedColors.length;
  }
  if (orderData.factoryProduction && typeof orderData.factoryProduction === 'object') {
    const keys = Object.keys(orderData.factoryProduction).filter(k => k && String(k).trim() !== '');
    if (keys.length > 0) return keys.length;
  }
  return 0;
};

const extractOrderSizes = (orderData, lookupSizes = []) => {
  if (!orderData) return [];

  // 1. Manual sizes array
  if (Array.isArray(orderData.manualSizes) && orderData.manualSizes.length > 0) {
    const valid = orderData.manualSizes
      .map(s => (s !== null && s !== undefined ? String(s).trim() : ''))
      .filter(Boolean);
    if (valid.length > 0) return valid;
  }

  // 2. From sizeFrom and sizeTo using lookupSizes
  if (orderData.sizeFrom && orderData.sizeTo) {
    if (Array.isArray(lookupSizes) && lookupSizes.length > 0) {
      const idx1 = lookupSizes.indexOf(orderData.sizeFrom);
      const idx2 = lookupSizes.indexOf(orderData.sizeTo);
      if (idx1 !== -1 && idx2 !== -1) {
        return lookupSizes.slice(Math.min(idx1, idx2), Math.max(idx1, idx2) + 1);
      }
    }
    return [orderData.sizeFrom, orderData.sizeTo];
  }

  // 3. Extract unique size keys from colorDistribution
  const distSizes = new Set();
  if (orderData.colorDistribution && typeof orderData.colorDistribution === 'object') {
    let hasQuantities = false;
    Object.values(orderData.colorDistribution).forEach(sizesMap => {
      if (sizesMap && typeof sizesMap === 'object') {
        Object.values(sizesMap).forEach(v => {
          if (parseInt(v, 10) > 0) hasQuantities = true;
        });
      }
    });
    ...
    Object.values(orderData.colorDistribution).forEach(sizesMap => {
      if (sizesMap && typeof sizesMap === 'object') {
        Object.entries(sizesMap).forEach(([s, v]) => {
          if (s && String(s).trim()) {
            if (hasQuantities) {
              if (parseInt(v, 10) > 0) distSizes.add(String(s).trim());
            } else {
              distSizes.add(String(s).trim());
            }
          }
        });
      }
  ...
  // 4. Extract unique size keys from groupedMeasurements
  if (orderData.groupedMeasurements && typeof orderData.groupedMeasurements === 'object') {
    Object.values(orderData.groupedMeasurements).forEach(partMap => {
      if (partMap && typeof partMap === 'object') {
        Object.values(partMap).forEach(mMap => {
          if (mMap && typeof mMap === 'object') {
            Object.keys(mMap).forEach(s => {
              if (s && String(s).trim()) distSizes.add(String(s).trim());
            });
          }
        });
      }
  ...
  // 5. Extract from old measurements
  if (orderData.measurements && typeof orderData.measurements === 'object') {
    Object.values(orderData.measurements).forEach(mMap => {
      if (mMap && typeof mMap === 'object') {
        Object.keys(mMap).forEach(s => {
          if (s && String(s).trim()) distSizes.add(String(s).trim());
        });
  ...
  // Guard against accidental full-table dumps where no sizes were chosen,
  // but all system sizes (e.g. 20+ sizes spanning children, adult, letter) got saved.
  if (distSizes.size >= 20 || (Array.isArray(lookupSizes) && lookupSizes.length > 0 && distSizes.size >= lookupSizes.length)) {
    return [];
  }

  if (distSizes.size > 0) {
    return Array.from(distSizes);
  }

  if (orderData.sizeFrom) return [orderData.sizeFrom];
  if (orderData.sizeTo) return [orderData.sizeTo];
  if (Array.isArray(orderData.sizes) && orderData.sizes.length > 0) return orderData.sizes;
  if (typeof orderData.sizes === 'string' && orderData.sizes.trim()) {
    return orderData.sizes.split(/[,،\n]+/).filter(Boolean);
  }
  return [];
};

const sortSizes = (sizesList) => {
  if (!sizesList || sizesList.length === 0) return [];
  const sizeOrderArr = ['XXS', 'XS', 'S', 'M', 'L', 'XL', 'XXL', 'XXXL', '2XL', '3XL', '4XL', '5XL', '6XL', 'F', 'FREE'];
  return [...sizesList].sort((a, b) => {
    const strA = String(a).trim().toUpperCase();
    const strB = String(b).trim().toUpperCase();
    const ai = sizeOrderArr.indexOf(strA);
    const bi = sizeOrderArr.indexOf(strB);
...
const getSizesCount = (orderData, lookupSizes = []) => {
  if (!orderData) return 0;
  if (orderData.numberOfSizes && parseInt(orderData.numberOfSizes, 10) > 0) {
    return parseInt(orderData.numberOfSizes, 10);
  }
  const sizes = extractOrderSizes(orderData, lookupSizes);
  return sizes.length;
};

const getSizeRange = (orderData, lookupSizes = []) => {
  if (!orderData) return '-';
  ...
  const allSizes = extractOrderSizes(orderData, lookupSizes);
  ...
  const sorted = sortSizes(allSizes);
  ...
  const exportToExcel = (includePrices = true) => {
    ...
    const excelData = filteredOrders.map(o => {
      ...
      const row = {
        ...
        [t('reports.excel_headers.barcode', { defaultValue: 'الباركود' })]: d.barcode || d.productBarcode || '-',
        ...
        [t('reports.excel_headers.colors_count', { defaultValue: 'عدد الألوان' })]: getColorsCount(d) || '-',
        [t('reports.excel_headers.sizes_count', { defaultValue: 'عدد المقاسات' })]: getSizesCount(d, lookups.sizes) || '-',
        [t('reports.excel_headers.sizes')]: getSizeRange(d, lookups.sizes),
  ...
  const exportToPDF = async (includePrices = true) => {
    ...
    try {
       ...
       const rows = filteredOrders.map((order, idx) => {
         ...
         gQty += qty;
         gAmt += tp;
         const rawBc = d.barcode || d.productBarcode || '';
         const bc = rawBc ? String(rawBc).split(/[\n,]/)[0].trim() : '';
         const clrs = getColorsCount(d);
         const sc = getSizesCount(d, lookups.sizes);
         const sr = getSizeRange(d, lookups.sizes);
      ...
      // Standard contract columns - always preserved, showing '-' when empty
      const columns = [
        ...
        ...(rows.some(r => r.cn && String(r.cn).trim() !== '' && String(r.cn).trim() !== '-') ? [
      ...
      const activeCols = columns;
       ...
       if (hasReqDate || hasDelDate) {
        ...
        const minCondHeight = maxItemsCount > 0 ? Math.max(110, maxItemsCount * 22 + 15) : 90;

        let conditionsHtml = 
          `<table style="border-collapse:collapse;width:100%;border:2px solid #000;border-top:none;"><tbody>
            <tr>
              <td colspan="2" style="${bl}background:#e6e6e6;font-weight:800;font-size:13px;padding:5px 12px;">
                Conditions 状况：
              </td>
            </tr>
            <tr>
              <td style="${bl}width:50%;height:${minCondHeight}px;vertical-align:top;padding:8px 14px;border-right:1px solid #000;">
                ${finalCol1.length > 0 ? `
                  <ul style="margin:0;padding-left:16px;font-size:11.5px;font-weight:bold;line-height:1.45;">
                    ${finalCol1.map(t => `<li style="margin-bottom:4px;">${t}</li>`).join('')}
                  </ul>
                ` : '&nbsp;'}
              </td>
              <td style="${bl}width:50%;height:${minCondHeight}px;vertical-align:top;padding:8px 14px;">
                ${finalCol2.length > 0 ? `
                  <ul style="margin:0;padding-left:16px;font-size:11.5px;font-weight:bold;line-height:1.45;">
                    ${finalCol2.map(t => `<li style="margin-bottom:4px;">${t}</li>`).join('')}
                  </ul>
                ` : '&nbsp;'}
              </td>
            </tr>
       ...
       const signatureHtml = 
         '<table style="border-collapse:collapse;width:100%;border:2px solid #000;border-top:none;"><tbody><tr>'+
           '<td style="'+bl+'width:25%;padding:10px 10px;">'+
             '<div style="font-weight:800;font-size:13px;margin-bottom:16px;">Name 名字</div>'+
             '<div style="font-weight:800;font-size:13px;">Signature 签名</div>'+
           '</td>'+
           '<td style="'+tc+'width:25%;padding:10px 10px;vertical-align:top;">'+
             '<div style="color:#b41e1e;font-weight:800;font-size:13px;margin-bottom:16px;">Buyer 买方</div>'+
             '<div style="border-bottom:2px solid #000;width:80%;margin:0 auto;height:20px;"></div>'+
           '</td>'+
           '<td style="'+tc+'width:25%;padding:10px 10px;vertical-align:top;">'+
             '<div style="color:#b41e1e;font-weight:800;font-size:13px;margin-bottom:16px;">Coordinator 协调员</div>'+
             '<div style="border-bottom:2px solid #000;width:80%;margin:0 auto;height:20px;"></div>'+
           '</td>'+
           '<td style="'+tc+'width:25%;padding:10px 10px;vertical-align:top;">'+
             '<div style="color:#b41e1e;font-weight:800;font-size:13px;margin-bottom:16px;">Factory 工厂</div>'+
             '<div style="border-bottom:2px solid #000;width:80%;margin:0 auto;height:20px;"></div>'+
           '</td>'+
         '</tr></tbody></table>';
       ...
       // ─── PAGINATION: Split rows into pages (18 rows per page for perfect A4 fit) ───
       ...
       for (let pageIdx = 0; pageIdx < totalPages; pageIdx++) {
         ...
         // Pad empty rows up to 18 rows per page
         ...
         // 1050px width x 1485px height matches standard A4 portrait ratio (1 : 1.4142857)
         const pageEl = document.createElement('div');
         pageEl.style.cssText = 'position:fixed;left:-9999px;top:0;width:1050px;min-height:1485px;box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;background:#fff;padding:20px 24px;font-family:"Microsoft YaHei",SimHei,SimSun,Inter,sans-serif;color:#000;font-size:14px;line-height:1.35;direction:ltr;text-align:left;-webkit-font-smoothing:antialiased;';
         ...
         // Title
         pageHtml += '<div style="text-align:center;margin-bottom:10px;">';
         pageHtml += '<span style="font-size:32px;font-weight:900;color:#b41e1e;letter-spacing:1px;">Order Contract 订单合同</span>';
         if (totalPages > 1) {
           pageHtml += `<span style="font-size:13px;color:#666;margin-right:10px;display:block;margin-top:3px;">Page ${pageIdx + 1} / ${totalPages}</span>`;
         }
         pageHtml += '</div>';
         ...
         const marginMM = 4;
         const maxContentWidthMM = pdfWidthMM - marginMM * 2; // 202mm
         const maxContentHeightMM = pdfHeightMM - marginMM * 2; // 289mm
         ...
         const posX = (pdfWidthMM - renderWidthMM) / 2;
         const posY = (pdfHeightMM - renderHeightMM) / 2;
                               ...
                               <div className="expandable-content-wrapper">
                                 ...
                                 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '1.5rem', padding: '1.5rem', background: 'rgba(255,255,255,0.03)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(255,255,255,0.05)' }}>
                                    <div>
                                       ...
                                       {(() => {
                                           const range = getSizeRange(d, lookups.sizes);
                                          if (range && range !== '-') {
